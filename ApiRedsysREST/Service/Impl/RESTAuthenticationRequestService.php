			...
			if (is_null($response->getApiCode())) {
				...
				if(!$this->checkSignature($paramsB64, $response->getOperation()->getSignature()))
				{
					...
					switch ((int)$response->getOperation()->getResponseCode()){
						case RESTConstants::$AUTHORIZATION_OK: $response->setResult(in_array($transType, [RESTConstants::$AUTHORIZATION, RESTConstants::$PREAUTHORIZATION, RESTConstants::$VALIDATION]) ? RESTConstants::$RESP_LITERAL_OK : RESTConstants::$RESP_LITERAL_KO); break;
						case RESTConstants::$CONFIRMATION_OK: $response->setResult(in_array($transType, [RESTConstants::$CONFIRMATION, RESTConstants::$REFUND, RESTConstants::$VALIDATION_CONFIRMATION]) ? RESTConstants::$RESP_LITERAL_OK : RESTConstants::$RESP_LITERAL_KO); break;
						case RESTConstants::$CANCELLATION_OK: $response->setResult(in_array($transType, [RESTConstants::$CANCELLATION]) ? RESTConstants::$RESP_LITERAL_OK : RESTConstants::$RESP_LITERAL_KO); break;
